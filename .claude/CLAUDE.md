...
* **Locale typing in layouts vs pages**: Next.js enforces `LayoutConfig<Route>` strictly on layout files — `params` **must** be typed as `{ locale: string }` (not `Locale`). Use `getDictionary(locale as Locale)` inside the layout body. Page files use a looser `PageConfig` check and may declare `{ locale: Locale }` directly (as seen in `auth/login`, `cart`, etc.). Never use `locale: Locale` in a `layout.tsx` params type — it will fail at Vercel build time.
* **Navbar placement**: The `<Navbar>` lives in `src/app/[locale]/layout.tsx` — **never** inside individual page components. Placing it in the layout ensures it is preserved across SPA navigations, preventing the logo and client-side components (ThemeToggle, LanguageSwitcher) from unmounting and remounting on every route change. The `auth/layout.tsx` subtree renders below the Navbar naturally; auth pages just don't show a second logo.
...
* **Forms that navigate**: Never use `<form method="get">` for in-app search or filter navigation — a native form submission is a full page reload, which puts the source page in bfcache and kills React event listeners. Instead, make the form a `'use client'` component and pass a `(formData: FormData) => void` function to the `action` prop — React 19 intercepts it, passes the `FormData` directly, and no `e.preventDefault()` is needed. See `SearchBar.tsx` as the canonical example.
